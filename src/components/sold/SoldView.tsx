...
import { PackageCheck, Pencil, RefreshCw, Search, ShoppingCart, Trash2, Warehouse } from 'lucide-react';
...
export const SoldView: React.FC = () => {
  ...
  const returnToWarehouse = async (sale: SaleHistoryItem) => {
    if (!window.confirm(`Return ${sale.serialNumber} to its previous warehouse? This will remove it from sold items.`)) return;
    setSaving(true);
    try {
      ...
      addNotification('success', 'Returned to Warehouse', `${sale.serialNumber} returned to ${result.location}.`);
      triggerRefresh();
    } catch (error: any) {
      addNotification('error', 'Return Failed', error.message || 'Could not return item to its warehouse.');
    } finally {
      setSaving(false);
    }
  };
